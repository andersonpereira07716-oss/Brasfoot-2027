...
export default function App() {
  ...
  if (showPitch) {
    return (
      <div className="relative min-h-screen bg-slate-950">
        {/* Botão flutuante para voltar ao jogo */}
        <button 
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none">
      {/* Header com o nome e o botão de Proposta Comercial */}
      ...
      <main className="p-4 space-y-4 max-w-md mx-auto w-full">
        ...
        <div className="flex space-x-1.5 bg-slate-900 p-1 rounded-xl border border-slate-800 text-xs font-semibold overflow-x-auto">
          <button className="flex-1 bg-blue-600 text-white py-2 rounded-lg shadow">Tabela</button>
          <button className="flex-1 text-slate-400 hover:text-white py-2 rounded-lg">Elenco</button>
          <button className="flex-1 text-slate-400 hover:text-white py-2 rounded-lg">Tática 📋</button>
          <button className="flex-1 text-slate-400 hover:text-white py-2 rounded-lg">Mercado</button>
          <button className="flex-1 text-slate-400 hover:text-white py-2 rounded-lg">Notícias</button>
        </div>
